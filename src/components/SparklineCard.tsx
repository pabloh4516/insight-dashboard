...
function Sparkline({ data, color, height = 36, width = 100 }: { data: number[]; color: string; height?: number; width?: number }) {
  ...
  return (
    <svg width={width} height={height} className="overflow-visible">
      ...
      <polygon points={areaPoints} fill={`url(#spark-fill-${color.replace(/[^a-z0-9]/gi, '')})`} />
...
export function SparklineCard({ title, value, icon: Icon, trend, sparkData, color, compact = false }: SparklineCardProps) {
  if (compact) {
    return (
      <div className="flex items-center gap-3 border rounded-lg px-3 py-2.5 bg-card card-hover">
        <Icon className="h-3.5 w-3.5 text-muted-foreground" />
        <span className="text-xs text-muted-foreground">{title}</span>
        <span className="text-sm font-bold text-foreground ml-auto font-mono">{value.toLocaleString()}</span>
  ...
  return (
    <div
      className="border rounded-lg p-4 bg-card card-hover group"
    ...
    >
      ...
      <div className="flex items-end justify-between">
        <div>
          <div className="text-2xl font-bold text-foreground font-mono">{value.toLocaleString()}</div>
          <TrendIndicator value={trend} />
        </div>
...
function TrendIndicator({ value }: { value: number }) {
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 text-[10px] mt-1 ${isPositive ? "text-primary" : "text-error"}`}>
