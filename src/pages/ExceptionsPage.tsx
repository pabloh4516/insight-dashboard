...
const columns = [
  { key: 'class', header: 'Tipo do Erro', render: (e: ExceptionEntry) => <span className="text-error font-semibold">{e.class.split('\\').pop()}</span> },
  ...
  { key: 'occurrences', header: 'Vezes', render: (e: ExceptionEntry) => <span className={`font-semibold ${e.occurrences > 3 ? 'text-error' : 'text-warning'}`}>{e.occurrences}x</span> },
    ...
    <DataTable
      ...
      expandable={(e) => (
        <div className="space-y-3 text-xs">
          <div><span className="text-muted-foreground">Tipo completo:</span> <span className="text-error">{e.class}</span></div>
          <div><span className="text-muted-foreground">Arquivo:</span> <span className="text-info">{e.file}:{e.line}</span></div>
          <div>
            <span className="text-muted-foreground">Rastreamento:</span>
            <pre className="mt-1 bg-muted/30 p-3 rounded text-[10px] overflow-auto text-warning leading-relaxed">
              {e.stackTrace.join('\n')}
            </pre>
          </div>
        </div>
